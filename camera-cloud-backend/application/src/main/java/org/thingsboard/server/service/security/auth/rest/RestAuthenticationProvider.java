...
public class RestAuthenticationProvider implements AuthenticationProvider {
    ...
    private Authentication authenticateByUsernameAndPassword(Authentication authentication, UserPrincipal userPrincipal, String username, String password) {
        ...
        User user = userMngService.findUserByUserName(username);
        if (user == null) {
            User userSys = userService.findUserByEmail(TenantId.SYS_TENANT_ID, username);
            if (userSys != null && isAdminSystem(userSys.getId())) {
                user = userSys;
            } else if (userSys == null) {
        ...
        try {
            ...
            List<String> permissionNames = roleService.findPermissionByUserId(user.getId());

            SecurityUser securityUser = new SecurityUser(user, userCredentials.isEnabled(), userPrincipal);

            logLoginAction(user, authentication, ActionType.LOGIN, null);
            Collection<GrantedAuthority> authorities = permissionNames.stream().map(authority -> new SimpleGrantedAuthority(authority))
                    .collect(Collectors.toList());
            authorities.addAll(securityUser.getAuthorities());
