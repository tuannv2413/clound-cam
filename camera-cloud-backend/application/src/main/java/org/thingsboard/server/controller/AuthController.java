...
public class AuthController extends BaseController {
    ...
    public JwtTokenPair resetPassword(
        ...
        try {
            ...
            if (userCredentials != null) {
                ...
                User user = userService.findUserById(TenantId.SYS_TENANT_ID, userCredentials.getUserId());
                if (user != null && !isAdminSystem(user.getId()) && userMngService.findByTbUserId(user.getId().getId()) == null) {
                    throw new ThingsboardException("User not active", ThingsboardErrorCode.USER_NOT_ACTIVE);
                }
                String encodedPassword = passwordEncoder.encode(password);
    ...
    @ApiOperation(value = "Get login User (getUserLogin)",
            notes = "Get the information about the User which credentials are used to perform this REST API call.")
    @PreAuthorize("isAuthenticated()")
    @RequestMapping(value = "/auth/get-login-user", method = RequestMethod.GET)
    public @ResponseBody
    ResponseEntity<?> getUserLogin() throws ThingsboardException {
        try {
            UserLoginDto userLoginDto = null;
            SecurityUser securityUser = getCurrentUser();
            User user =  checkNotNull(userService.findUserById(securityUser.getTenantId(), securityUser.getId()));
            userLoginDto = userMngService.findUserByUserId(user.getId());
            if (userLoginDto == null) {
                UserLoginDto userLoginConvert = new UserLoginDto(user);
                if (user.getEmail().equals("sysadmin@thingsboard.org")) {
                    List<String> permisionNames = permissionService.findAll().stream().map(p -> p.getPermission()).collect(Collectors.toList());
                    userLoginConvert.setPermissionNames(permisionNames);
                    userLoginConvert.setRoles(new ArrayList<>());
                } else {
                    userLoginConvert.setRoles(roleService.findRolesByUserId(user.getId()));
                    userLoginConvert.setPermissionNames(roleService.findPermissionByUserId(user.getId()));
                }
                return new ResponseEntity<>(userLoginConvert, HttpStatus.OK);
            }
    ...
    @ApiOperation(value = "Request reset password email (requestResetPasswordByEmail)",
            notes = "Request to send the reset password email if the user with specified email address is present in the database. " +
                    "Always return '200 OK' status for security purposes.")
    @RequestMapping(value = "/noauth/recoverPasswordByEmail", method = RequestMethod.POST)
    @ResponseStatus(value = HttpStatus.OK)
    public void recoverPasswordByEmail(
        ...
        try {
            String email = resetPasswordByEmailRequest.getEmail();
            User user = checkNotNull(userService.findUserByEmail(null, email));
            UserCredentials userCredentials = checkNotNull(userService.requestPasswordReset(TenantId.SYS_TENANT_ID, email));
            AdminSettings adminSettings = adminSettingService.findAdminSettingsByKey(user.getTenantId(), "web-admin");
            JsonNode jsonNode =  checkNotNull(adminSettings.getJsonValue());
            List<String> urlMailClient =  jsonNode.findValuesAsText("url");
            String resetUrl = String.format("%s/auth/reset-password/%s", urlMailClient.get(0),
                    userCredentials.getResetToken());
            ...
            mailService.sendRecoverPasswordAsync(resetUrl, email, fullName.trim());
        } catch (Exception e) {
            throw handleException(e);
        }
    }
    @ApiOperation(value = "Refressh toke (refreshToken)",
            notes = "Checks the password reset token and updates the password. " +
                    "If token is valid, returns the object that contains [JWT](https://jwt.io/) access and refresh tokens. " +
                    "If token is not valid, returns '404 Bad Request'.")
    @RequestMapping(value = "/noauth/refreshToken", method = RequestMethod.POST)
    @ResponseStatus(value = HttpStatus.OK)
    @ResponseBody
    public JwtTokenPair refressToken(@ApiParam(value = "The refresh token string.")
                                     @RequestBody RefreshTokenRequest refreshTokenRequest,
                                     HttpServletRequest request) {
        SecurityUser securityUser = tokenFactory.parseAccessJwtToken(new RawAccessJwtToken(refreshTokenRequest.getRefreshToken()));
        JwtToken accessToken = tokenFactory.createAccessJwtToken(securityUser);
        JwtToken refresh = refreshTokenRepository.requestRefreshToken(securityUser);
        return new JwtTokenPair(accessToken.getToken(), refresh.getToken());

    }

    private boolean isAdminSystem(UserId userId){
        return userMngService.checkAdminSystem(userId.getId());
    }
